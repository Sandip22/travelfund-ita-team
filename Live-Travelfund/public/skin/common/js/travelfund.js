...
validation = {
	...
	bind : function() {
		...
		$.each(this.cRule, function(field) {
                    var elementType = $("#" + field).prop('type'); //get type of element
	...
	required : function(oEle) {
		...
		var exp = /^[a-zA-Z][^0-9]+$/; //First character must be a letter & no number in between
		...
		var exp = /^([A-PR-UWYZ0-9][A-HK-Y0-9][AEHMNPRTVXY0-9]?[ABEHMNPRVWXY0-9]? {1,2}[0-9][ABD-HJLN-UW-Z]{2}|GIR 0AA)$/; //- Max length will be 11 characters. (or 13 with two spaces.)
		if (!oEle.val.match(exp)) {
			var oMsg = {};
			//oMsg.str = oEle.label + " must have only alphabet";
                        oMsg.str = oEle.label;
		...
		var exp = /^[0-9 ]{11,13}$/; //- Max length will be 11 characters. (or 13 with two spaces.)
                if (!oEle.val.match(exp)) {
			var oMsg = {};
			//oMsg.str = oEle.label + " must have only alphabet";
                        oMsg.str = oEle.label;
		...
		var exp = /^[\s\S]{0,50}$/; //Maxlength & Regular Expressions for text boxes 50 char
		if (!oEle.val.match(exp)) {
			var oMsg = {};
			//oMsg.str = oEle.label + " must have only alphabet";
                        oMsg.str = oEle.label;
		...
		var exp = /^[a-zA-Z '-A-Z '-]/; //allow Hyphens & Apostrophes  e.g. D’Souza
		if (!oEle.val.match(exp)) {
			var oMsg = {};
			//oMsg.str = oEle.label + " must have only alphabet";
                        oMsg.str = oEle.label;
    ...
    alphaNumeric : function(oEle) {
		...
		var exp = /^[a-zA-Z0-9_]*$/; //alpshnumeric with no space
		if (!oEle.val.match(exp)) {
			var oMsg = {};
	...
	match : function(oEle) {
            ...
            console.log(oEle);
            console.log($("#" + oEle.id).val().toLowerCase());
            console.log($("#" + oEle.match_id).val().toLowerCase());
       ...
       validEmail : function(oEle) {
		...
		//var numExp = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/; // not allow + sign
	...
	sValidate : function(eve) {
		...
		if ($("#err_" + pData.field)) {
				$("#err_" + pData.field).remove();
		}
		$("#" + pData.field + "_yes").addClass("hidden");
		...
		var jqxhr = $.ajax({
			...
			if (respo.error) {
				...
				oMsg.id = pData.field;
				$("#" + pData.field).after(validation.get_err(oMsg));
			} else if(respo.right) {
				$("#" + pData.field + "_yes").removeClass("hidden");
			}
		}).fail(function(respo) {
			console.log(respo);
		});
